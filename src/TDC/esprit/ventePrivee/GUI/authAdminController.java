...
//       int log=Integer.parseInt(fieldLogin.getText());
        ...
        String req="select * from admin where login_admin=? and pwd_admin=?";
        // requete authentification User
        String reqUser="select * from utilisateur where login_user=? and pwd_user=?";
        ...
        try {  
          ...
          // cas User
            pstUser=c.prepareStatement(reqUser); 
            pstUser.setString(1,fieldLogin.getText());
            pstUser.setString(2,field_pwd.getText());
            resUser=pstUser.executeQuery();
            ...
            root=FXMLLoader.load(getClass().getResource("AdminSpace.fxml"));
              ...
              //create a new scene with root and set the stage// Creer un nouveau scène 
                 ...
                 stage.show();
                    ...
                    loginUser=fieldLogin.getText();
                    pwdUser=field_pwd.getText();
                    
              if (event.getSource()==btnConnect){
                  
                  idUser=resUser.getInt("id_user");
                     //get reference to the button's stage
            stage=(Stage) btnConnect.getScene().getWindow();
             //load up OTHER FXML document
